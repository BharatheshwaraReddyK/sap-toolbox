...
export interface DiffReportData {
  ...
  stats: LineDiffStats
  lines: LineEntry[]
  /** Structural, path-level differences (optional — CSV reports use row-level info instead). */
  fields?: FieldRow[]
...
h3{font-size:0.95rem;margin:24px 0 4px;}
.section-subtitle{color:#5B6478;font-size:0.78rem;margin:0 0 10px;font-family:ui-monospace,monospace;}
table.fields{width:100%;border-collapse:collapse;font-size:0.78rem;margin-bottom:8px;}
table.fields th{text-align:left;padding:7px 10px;border-bottom:2px solid #DCE1EC;color:#5B6478;font-weight:600;background:#F8F9FC;}
table.fields td{padding:6px 10px;border-bottom:1px solid #E7EAF3;vertical-align:top;}
table.fields td.sym{width:26px;font-weight:700;text-align:center;}
table.fields td.label{width:100px;font-weight:600;white-space:nowrap;}
table.fields code{font-family:ui-monospace,"JetBrains Mono",monospace;font-size:0.75rem;}
...
function fieldRowHtml(r: FieldRow): string {
  return `<tr>
...
export function buildDiffReportBody(data: DiffReportData): string {
  const { title, generatedAt, labelA, labelB, rawA, rawB, stats, lines, fields, fieldsSummary, note } = data

  const fieldsSection = fields
    ? `<h3>Field-level differences</h3>
${fieldsSummary ? `<p class="section-subtitle">${escapeHtml(fieldsSummary)}</p>` : ''}
${
  fields.length
    ? `<table class="fields">
    ...
    : `<p class="section-subtitle">No field-level differences.</p>`
}`
    : ''
...
/**
 * Dynamically imports a code-split chunk, recovering from the "stale deployment" failure mode:
 * if the currently-loaded page is from an older build, the hashed chunk filename it asks for
 * may no longer exist on the server after a redeploy. In that case we reload the page once
 * (picking up the current build) rather than surfacing a raw fetch error.
 */
async function importChunk<T>(loader: () => Promise<T>): Promise<T> {
  try {
    return await loader()
  } catch (err) {
    ...
    if (!sessionStorage.getItem(reloadKey)) {
      ...
      // Block forever — the reload will replace this whole script context.
      return new Promise<T>(() => {})
    }
    throw new Error(
      err instanceof Error && /fetch/i.test(err.message)
        ? 'Could not load the export module. This tab may be running an outdated version of the app — please refresh the page and try again.'
